...
export interface IStorage {
  ...
  updateChatHistory(userId: number, deviceId: string, chatHistory: Partial<ChatHistory>): Promise<ChatHistory>;
}
...
export class MemStorage implements IStorage {
  ...
  async getChatHistory(userId: number, deviceId: string): Promise<ChatHistory | undefined> {
    return Array.from(this.chatHistoryMap.values()).find(
      (history) => history.userId === userId && history.deviceId === deviceId
    );
  }
  ...
  async updateChatHistory(userId: number, deviceId: string, updatedHistory: Partial<ChatHistory>): Promise<ChatHistory> {
