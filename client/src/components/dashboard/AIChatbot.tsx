...
const AIChatbot = ({ userProfile }: AIChatbotProps) => {
  ...
  const [deviceId] = useState(() => getDeviceId()); // Get or generate a unique device ID
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Fetch existing chat history for this device
  const { data: chatHistory, isLoading } = useQuery({
    queryKey: ['/api/chat/history', deviceId],
    queryFn: async () => {
      const response = await apiRequest('GET', `/api/chat/history?deviceId=${deviceId}`);
  ...
  const sendMessage = useMutation({
    mutationFn: async (message: string) => {
      const response = await apiRequest('POST', '/api/chat/message', { 
        message,
        deviceId  // Include deviceId with every message
      });
      return response.json();
    },
    onSuccess: (data) => {
